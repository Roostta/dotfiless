...
# Rofi
bindsym $mod+Tab   exec --no-startup-id "rofi -x11 -show"
# bindsym $mod+g     exec --no-startup-id "rofi -x11 -show calc -modi calc -no-show-match -no-sort -calc-command \\"echo -n '{result}' | xclip -selection clipboard\\""
bindsym $mod+y     exec --no-startup-id "rofi -x11 -show clipmenu"
bindsym $mod+grave exec --no-startup-id rofi -x11 -show window
